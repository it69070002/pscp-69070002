...
2
constraints 
เข้าใจว่าต้องเริ่มจาก a และลดลงทีละ 2 จนกว่าจะถึงค่าที่มากกว่า 0 
```
...
```text
Step 1: อ่านจำนวนเต็ม a และ b
Step 2: กำหนดตัวแปร total = 0 เพื่อเก็บผลรวม
Step 3: เริ่มจาก a แล้วลดค่าทีละ 2 
Step 4: นำค่าที่ได้แต่ละตัวมาบวกเพิ่มใน total
Step 5: หลังจากบวกแต่ละตัว ให้ตรวจว่า total >= b หรือไม่
Step 6: ถ้า total >= b ให้หยุดการทำงานและแสดงจำนวนตัวเลขที่ใช้บวก
Step 7: ถ้าลองบวกตัวเลขที่มีทั้งหมดแล้ว total ยังน้อยกว่า b ให้แสดง -1
ก่อนเขียนโค้ด คิดว่าน่าจะใช้ for loop เพราะรู้ว่าต้องวนค่าจาก a ลดลงทีละ 2 และใช้ตัวแปรสะสมเพื่อเก็บผลรวม แต่ยังไม่แน่ใจว่าจะคำนวณจำนวนรอบที่ต้องใช้ให้ง่ายที่สุดอย่างไร
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก เพราะยังใช้การวนลูปเริ่มจาก a แล้วลดค่าทีละ 2 และบวกค่าที่ได้สะสมไว้ใน total จากนั้นตรวจทุกครั้งว่าผลรวมถึงหรือเกิน b แล้วหรือยัง ถ้าถึงแล้วจะแสดงจำนวนรอบที่ใช้ แต่ถ้าวนครบแล้วยังไม่ถึง b จะแสดง -1 วิธีนี้ตรงกับแผนแรกและสามารถนำไปเขียนเป็น code ได้ตามที่ตั้งใจไว้ตั้งแต่ต้น
```
...
```text
 ตรวจสอบกรณีที่วนครบทุกค่าแล้ว แต่ผลรวมยังน้อยกว่า b ซึ่งควรแสดง -1
```
...
```text
รวจสอบกรณีที่ total มีค่าเท่ากับ b พอดี ไม่ได้มากกว่า b
```
...
```text
ตรวจสอบว่าถ้า a ตัวแรกก็เพียงพอที่จะทำให้ผลรวมถึง b โปรแกรมจะหยุดทันทีและนับเป็น 1 ครั้ง
```
